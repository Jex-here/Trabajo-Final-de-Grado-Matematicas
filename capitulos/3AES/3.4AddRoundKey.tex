\textit{AddRoundKey} toma como parámetros el \textit{estado} y la clave $k_i$ de la ronda correspondiente, la cual se compone de 128 bits y se expresa en una matriz $4 \times 4$, de la misma manera en la que se expresó el \textit{estado} al inicializarse el algoritmo de AES. Así, \textit{AddRoundKey} consiste en sumar celda a celda ambas matrices, donde la suma se efectúa sobre $\F_{2^8}$. Esta operación \begin{Revision}es sinónimo de efectuar la suma en $\Z_2$ bit a bit\end{Revision}, en ambas cadenas de 128 bits.
