...
\begin{dem}{\cite[p.~412]{GamboaAnillos}}
    ...
    \end{array}
    \]
    vuelve a ser biyectiva, pues la sobreyectividad vuelve a ser trivial, y la inyectividad se prueba de manera idéntica. Se tiene que si $\F = \gen{1,a_1,a_2}$, entonces $\F$ tiene $p^3$ elementos. En caso contrario, se continúa hasta que $\F = \gen{1,a_1, a_2, \ldots, a_{n-1}}$, pues $\F$ es un conjunto finito, y en cada paso la demostración de la biyectividad se realiza de manera idéntica. Se obtiene por lo tanto que un cuerpo finito cualquiera, $\F$, debe tener $p^n$ elementos.
\end{dem}
